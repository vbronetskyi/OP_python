...
def countries_locations(countries:list)->list[tuple]:
    ...
    for country in countries:
        try:
            countries_with_locations.append((countries_in_world[country][0], countries_in_world[country][1], countries_in_world[country][2]))
        except KeyError:
            continue
...
def draw_on_map(countries: list[tuple]):
    ...
    """
    map_of_available_markets = folium.Map(location=[49.8171100, 24.0224973], zoom_start=5)

    fgroup1 = folium.FeatureGroup(name="Marks of available markets to your track")
    for country in countries:
        fgroup1.add_child(folium.Marker(location=[country[1], country[2]],popup=f"{country[0]}",icon=folium.Icon()))
    map_of_available_markets.add_child(fgroup1)
    map_of_available_markets.add_child(folium.TileLayer('cartodbdark_matter', name= 'Black theme'))
