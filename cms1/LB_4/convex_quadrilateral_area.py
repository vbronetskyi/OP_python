...
def four_lines_area(k1, c1, k2, c2, k3, c3, k4, c4):
    ...
    """
    if k1 == k2 == k3 or k1 == k2 == k4 or k2 == k3 == k4:
        return 0
    else:
        # Сreate pairs
    ...
    if (size[0] and size[1] and size[2] and size[3] and size[4] and size[5]) != None:
...
def quadrangle_area(a, b, c, d, f1, f2):
    ...
    """
    try:
        return round(math.sqrt((4 - math.pow(f1, 2) * math.pow(f2, 2) - math.pow((pow(b, 2) + math.pow(d, 2) - math.pow(c, 2)), 2) / 4)))
    except ValueError:
        return None
