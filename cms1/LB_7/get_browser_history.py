...
def get_chrome(con: sqlite3.Connection) -> list:
    ...
    id_tuples = c.fetchall()
    id_ur = dict()
    for i in id_tuples:
        id_ur[i[0]] = (i[1], i[2])

    c.execute("select url, visit_time, visit_duration from visits")
    results_url = c.fetchall()

    results = []
    for result in results_with_url_as_id:
        url = url_id[result[0]]
        date = datetime.datetime.fromtimestamp((result[1]) / 1000000 - 11644473600).str().split()
...
def get_chrome_os(user: str, os: str) -> list:
    ...
    if os == "Linux":
        ...
        con = sqlite3.connect(f'C:/Users/{user}/AppData/Local/Google/Chrome/User Data/Profile 2/History')
    elif os == "MacOS":
...
write_data_to_file(get_chrome_os('User', 'Windows'), 'history.txt')